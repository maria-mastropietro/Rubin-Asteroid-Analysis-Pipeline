#!/usr/bin/env python3
"""
Run the High-Order Fourier and Multiband Lomb-Scargle asteroid-period workflows side by side
and print/save a comparison of the resulting periods, plus a wrapper-level
reliability assessment and a color/taxonomy assessment based on the
Taxonomy.py workflow.

This wrapper assumes the following files are in the same directory:
  - Period_search_High_Order_Fourier.py
  - Period_search_Multiband_Lomb_Scargle.py
  - Taxonomy.py
  - this file: Asteroid_physical_properties.py
...
Key features:
  ...
  - compares the two best-fit periods
  - applies wrapper-level reliability rules:
      * both methods must succeed
      * enough observations in at least two gri bands
      * both Section 3.1 and Section 3.2 amplitudes must exceed threshold
      * periods must agree within a fractional tolerance
  - computes color offsets, color differences, and taxonomy using the
    Section 3.2-style color/taxonomy workflow and the Section 3.2 best period
"""
...
SECTION31_PATH = os.path.join(BASE_DIR, "Period_search_High_Order_Fourier.py")
...
class ReliabilityAssessment:
    ...
    n_good_bands: int
    both_methods_ok: bool
    enough_two_bands: bool
    amplitude_section31: Optional[float]
    amplitude_section32: Optional[float]
    amplitude_ok: bool
    period_fractional_difference: Optional[float]
    periods_agree: bool
    wrapper_reliable: bool
...
def assess_reliability(
    ...
    band_counts = load_band_counts(args.csv)
    n_good_bands = count_bands_meeting_threshold(
        band_counts,
        min_obs=args.min_obs_per_band,
        allowed=("g", "r", "i"),
    )

    both_methods_ok = (r31.status == "ok" and r32.status == "ok")
    enough_two_bands = (n_good_bands >= args.min_bands)

    amp31 = try_float(r31.extra.get("amplitude_mag"))
    amp32 = try_float(r32.extra.get("amplitude_mag"))

    amplitude_ok = (
        amp31 is not None and amp32 is not None and
        amp31 >= args.amplitude_threshold and
        amp32 >= args.amplitude_threshold
    )

    frac_diff = relative_period_difference(r31.best_period_days, r32.best_period_days)
    periods_agree = (frac_diff is not None and frac_diff <= args.period_agreement_tol)

    wrapper_reliable = (
        both_methods_ok
        and enough_two_bands
        and amplitude_ok
        and periods_agree
    )
    ...
    return ReliabilityAssessment(
        ...
        both_methods_ok=both_methods_ok,
        enough_two_bands=enough_two_bands,
        amplitude_section31=amp31,
        amplitude_section32=amp32,
        amplitude_ok=amplitude_ok,
        period_fractional_difference=frac_diff,
        periods_agree=periods_agree,
        wrapper_reliable=wrapper_reliable,
    )
...
def run_colors_taxonomy_for_period(
    ...
    with open(summary_txt, "w", encoding="utf-8") as f:
        ...
        if color_fit.color_errors is not None:
            for name in ["g-r", "g-i", "r-i", "i-z"]:
                val = try_float(color_fit.color_errors.get(name))
                f.write(f"color_{name.replace('-', '_')}_error={format_float(val, '.10f')}\n")
...
def write_comparison_csv(
    ...
    fieldnames = [
        ...
        "enough_two_bands",
        "n_good_bands",
        "section31_amplitude_mag",
        "section32_amplitude_mag",
        "amplitude_ok",
        "period_fractional_difference",
        "periods_agree",
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        ...
        for r in rows:
            row = {
                ...
                "wrapper_reliable": str(rel.wrapper_reliable),
                "both_methods_ok": str(rel.both_methods_ok),
                "enough_two_bands": str(rel.enough_two_bands),
                "n_good_bands": rel.n_good_bands,
                "section31_amplitude_mag": (
                    "" if rel.amplitude_section31 is None else f"{rel.amplitude_section31:.12f}"
                ),
                "section32_amplitude_mag": (
                    "" if rel.amplitude_section32 is None else f"{rel.amplitude_section32:.12f}"
                ),
                "amplitude_ok": str(rel.amplitude_ok),
                "period_fractional_difference": (
                    "" if rel.period_fractional_difference is None else f"{rel.period_fractional_difference:.12f}"
                ),
                "periods_agree": str(rel.periods_agree),
...
def write_comparison_txt(
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write(f"min_obs_per_band={args.min_obs_per_band}\n")
        f.write(f"min_bands={args.min_bands}\n")
        f.write(f"amplitude_threshold={args.amplitude_threshold}\n")
        f.write(f"period_agreement_tol={args.period_agreement_tol}\n")
        ...
        if len(rows) >= 2:
            ...
            if r1.best_period_hours is not None and r2.best_period_hours is not None:
                ...
                f.write(f"delta_hours={dh:.12f}\n")
            if rel.period_fractional_difference is not None:
                f.write(f"period_fractional_difference={rel.period_fractional_difference:.12f}\n")

        f.write("\n[Wrapper reliability]\n")
        f.write(f"band_counts={rel.band_counts}\n")
        f.write(f"n_good_bands={rel.n_good_bands}\n")
        f.write(f"both_methods_ok={rel.both_methods_ok}\n")
        f.write(f"enough_two_bands={rel.enough_two_bands}\n")
        f.write(
            f"section31_amplitude_mag="
            f"{'' if rel.amplitude_section31 is None else f'{rel.amplitude_section31:.12f}'}\n"
        )
        f.write(
            f"section32_amplitude_mag="
            f"{'' if rel.amplitude_section32 is None else f'{rel.amplitude_section32:.12f}'}\n"
        )
        f.write(f"amplitude_ok={rel.amplitude_ok}\n")
        f.write(
            f"period_fractional_difference="
            f"{'' if rel.period_fractional_difference is None else f'{rel.period_fractional_difference:.12f}'}\n"
        )
        f.write(f"periods_agree={rel.periods_agree}\n")
        f.write(f"wrapper_reliable={rel.wrapper_reliable}\n")
        ...
        f.write(f"Wrapper reliable: {table2_row.wrapper_reliable}\n")
...
def print_reliability(rel: ReliabilityAssessment, args: argparse.Namespace) -> None:
    print("\nReliability screening")
    print("---------------------")
    print(f"Band counts: {rel.band_counts}")
    print(f"Bands with >= {args.min_obs_per_band} obs in gri: {rel.n_good_bands}")
    print(f"Both methods succeeded: {rel.both_methods_ok}")
    print(f"Enough bands: {rel.enough_two_bands}")
    print(f"Section 3.1 amplitude: {format_float(rel.amplitude_section31, '.6f')}")
    print(f"Multiband Lomb-Scargle amplitude: {format_float(rel.amplitude_section32, '.6f')}")
    print(f"Amplitude >= {args.amplitude_threshold:.3f} mag: {rel.amplitude_ok}")
    print(
        "Fractional period difference: "
        f"{format_float(rel.period_fractional_difference, '.6f')}"
    )
    print(f"Periods agree within {args.period_agreement_tol:.1%}: {rel.periods_agree}")
    print(f"Wrapper reliable: {rel.wrapper_reliable}")
...
def summarize_table2_like(
    ...
    fourier_colors = build_fourier_colors(r31, color_tax_hof)

    h_mag = get_jpl_sbdb_h(args.target)
    if h_mag is None:
        h_mag = try_float(r31.extra.get("H_r"))
    ...
    return Table2LikeSummary(
        ...
        period_fourier_hours=r31.best_period_hours,
        period_fourier_error_hours=r31.period_error_hours,
        amplitude_lsm_mag=rel.amplitude_section32,
        amplitude_error_lsm_mag=r32.amplitude_error_mag,
        amplitude_fourier_mag=rel.amplitude_section31,
        amplitude_error_fourier_mag=r31.amplitude_error_mag,
        axial_elongation=compute_axial_elongation_from_amplitude(rel.amplitude_section32, color_tax.mean_alpha_deg),
        axial_elongation_error=compute_axial_elongation_error_from_amplitude(
            rel.amplitude_section32,
        ...
        color_gr_fourier=fourier_colors["g-r"],
        color_gr_lsm_error=try_float(color_tax.color_errors.get("g-r")),
        color_gr_fourier_error=try_float(color_tax_hof.color_errors.get("g-r")) if color_tax_hof.status == "ok" else None,
        color_gi_lsm=try_float(color_tax.colors.get("g-i")),
        color_gi_fourier=fourier_colors["g-i"],
        color_gi_lsm_error=try_float(color_tax.color_errors.get("g-i")),
        color_gi_fourier_error=try_float(color_tax_hof.color_errors.get("g-i")) if color_tax_hof.status == "ok" else None,
        color_ri_lsm=try_float(color_tax.colors.get("r-i")),
        color_ri_fourier=fourier_colors["r-i"],
        color_ri_lsm_error=try_float(color_tax.color_errors.get("r-i")),
        color_ri_fourier_error=try_float(color_tax_hof.color_errors.get("r-i")) if color_tax_hof.status == "ok" else None,
        color_iz_lsm=try_float(color_tax.colors.get("i-z")),
        color_iz_fourier=fourier_colors["i-z"],
        color_iz_lsm_error=try_float(color_tax.color_errors.get("i-z")),
        color_iz_fourier_error=try_float(color_tax_hof.color_errors.get("i-z")) if color_tax_hof.status == "ok" else None,
        ...
        gri_slope_fourier=try_float(color_tax_hof.taxonomy_info.get("gri_slope")) if color_tax_hof.status == "ok" else try_float(r31.extra.get("gri_slope")),
        gri_slope_fourier_error=try_float(color_tax_hof.taxonomy_info.get("gri_slope_error")) if color_tax_hof.status == "ok" else None,
        taxonomy_lsm=color_tax.taxonomy_info.get("taxonomy") if color_tax.status == "ok" else None,
        taxonomy_fourier=color_tax_hof.taxonomy_info.get("taxonomy") if color_tax_hof.status == "ok" else r31.extra.get("taxonomy"),
        wrapper_reliable=rel.wrapper_reliable,
    )
...
def print_table2_summary(row: Table2LikeSummary, color_tax: ColorTaxonomyAssessment) -> None:
    ...
    print(f"Wrapper reliable: {row.wrapper_reliable}")
...
def pair_fmt_str(a: Optional[str], b: Optional[str]) -> str:
    return f"{latex_escape(a or 'NA')} / {latex_escape(b or 'NA')}"
...
def main() -> None:
    ...
    file_must_exist(SECTION31_PATH, "Section 3.1 script")
    ...
    if args.import_check:
        load_module("period_search_high_order_fourier_mod", SECTION31_PATH)
        ...
        print(f"  Loaded: {SECTION31_PATH}")
        print(f"  Loaded: {SECTION32_PATH}")
        print(f"  Loaded taxonomy workflow: {COLORS_TAXONOMY_PATH}")
        return

    print("Running High-Order Fourier workflow...")
    r31 = run_section31(args)
    if r31.status.startswith("failed"):
        print("\n[Section 3.1 failed]")
        if "stderr" in r31.extra:
            print(r31.extra["stderr"])
        elif "stdout" in r31.extra:
            print(r31.extra["stdout"])
    ...
    print("Running color/taxonomy workflow (High-Order Fourier period)...")
    try:
        color_tax_hof = run_colors_taxonomy_hof(args, r31)
    except Exception as exc:
        color_tax_hof = ColorTaxonomyAssessment(
            summary_path=None,
            colors_csv_path=None,
    ...
    color_tax = overlay_taxonomy_plots_with_hof(args, color_tax, color_tax_hof)

    rows = [r31, r32]
    rel = assess_reliability(args, r31, r32)

    print_table(rows)
    print_reliability(rel, args)
    ...
    table2_row = summarize_table2_like(args, r31, r32, rel, color_tax, color_tax_hof)
    write_comparison_csv(comparison_csv, rows, rel, color_tax)
    write_comparison_txt(comparison_txt, rows, comparison_csv, args, rel, color_tax, table2_row)

    print(f"\nWrote comparison CSV: {comparison_csv}")
    print(f"Wrote comparison TXT: {comparison_txt}")

    if color_tax.status == "ok":
        if r31.extra.get("sigma_periodogram_png_path"):
            print(f"Wrote High-Order Fourier sigma plot: {r31.extra['sigma_periodogram_png_path']}")
        if r31.extra.get("phased_png_path"):
            print(f"Wrote High-Order Fourier phased plot: {r31.extra['phased_png_path']}")
        if r31.extra.get("residuals_png_path"):
            print(f"Wrote High-Order Fourier residual plot: {r31.extra['residuals_png_path']}")
        print(f"Wrote color/taxonomy summary: {color_tax.summary_path}")
        print(f"Wrote colors CSV: {color_tax.colors_csv_path}")
        if color_tax_hof.status == "ok":
            print(f"Wrote High-Order Fourier colors/taxonomy CSV: {color_tax_hof.colors_csv_path}")
