...
## Artifacts
The folder generated are:
- COMPARISON, where in {asteroid_name}_comparison.txt there is [Summary] with a summary of all period, amplitude, axial elongation, mean alpha used, colors, gri slope and taxonomy classification and wrapper reliable. 
- FIGURES
- HIGH_ORDER_FOURIER
- HORIZONS_CACHE
- MULTIBAND_LOMB_SCARGLE
- TAXONOMY
- TEX_TABLE


## Artifacts
The analysis generates the following output folders:

- COMPARISON: contains the final comparison files for each asteroid. In `{asteroid_name}_comparison.txt`, the `[Summary]` section provides a summary of the derived periods, amplitudes, axial elongation, mean phase angle, colors, `gri` slope, taxonomic classification, and the `wrapper_reliable` flag. The folder also contains a CSV version of the comparison results. 

- FIGURES: contains the plots generated by the analysis, including High-Order Fourier and Multiband Lomb-Scargle periodograms, phased lightcurves, residual plots, and the taxonomy-classification plot. 

- HIGH_ORDER_FOURIER: contains the numerical outputs from the High-Order Fourier analysis, including the interpolated Horizons data, the photometry merged with the ephemeris information, the best-period summary, the possible period solutions, and the best solution for each tested Fourier order. 

- HORIZONS_CACHE: contains cached JPL Horizons ephemeris queries. These files can be reused when the same target and observing interval are analyzed again, avoiding repeated Horizons queries.

- MULTIBAND_LOMB_SCARGLE: contains the numerical outputs from the Multiband Lomb-Scargle analysis, including the Horizons ephemeris table, the corrected and interpolated photometry, and the best-period summary.

- TAXONOMY: contains the color and taxonomic-classification results. This includes the analysis summary, derived colors and band offsets, taxonomic parameters and classification, and a per-band summary. 

- TEX_TABLE: contains the final Table-2-like summary of the asteroid properties in `.csv`, `.txt`, and `.tex` formats. These files collect the main physical and photometric results in a format convenient for inspection and use in LaTeX tables. 
