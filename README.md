...
## Artifacts
The following folders are generated by the asteroid analysis: 

- COMPARISON: contains the final comparison files for each asteroid. In `{asteroid_name}_comparison.txt`, the `[Summary]` section provides a summary of the derived periods, amplitudes, axial elongation, mean phase angle used, colors, `gri` slope, taxonomic classification, and the `wrapper_reliable` flag. The folder also contains a CSV version of the comparison results. 
...
- HIGH_ORDER_FOURIER: contains the numerical outputs from the High-Order Fourier analysis, including the interpolated Horizons data, the photometry merged with the ephemeris information, the best-period summary. 
...
- TAXONOMY: contains the color and taxonomic-classification results, including the analysis summary, derived colors and band offsets, taxonomic parameters and classification, and a per-band summary. 

- TEX_TABLE: contains the final Table-2-like summary of the asteroid properties in `.csv`, `.txt`, and `.tex` formats. 
