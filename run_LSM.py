"""
Run Asteroid_LSM.py for all objects in choose_lsm.txt.

This script:
1. Reads objects from choose_lsm.txt
2. Finds the corresponding *_ALL.csv files for each object
3. Runs Asteroid_LSM.py for each object
"""
...
# =========================
# Path to choose file
CHOOSE_FILE = Path(__file__).resolve().parent / "choose_lsm.txt"

# Path to Asteroid_LSM.py
LSM_SCRIPT = Path(__file__).resolve().parent / "Asteroid_LSM.py"
...
def run_lsm_for_object(
    ...
    """
    Run Asteroid_LSM.py for a single object.
    ...
    try:
        subprocess.run(
            cmd,
            cwd=str(Path(__file__).resolve().parent),  # Run in Asteroid_code folder
            check=True,
        )
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Horizons ephemeris step size in minutes",
    )
    args = parser.parse_args()

    print("=" * 80)
    print("Run Asteroid_LSM.py (LSM + taxonomy) for objects in choose_lsm.txt")
    print("=" * 80)

    if not LSM_SCRIPT.exists():
        print(f"[ERROR] Asteroid_LSM.py not found: {LSM_SCRIPT}")
        return
    ...
    if not objects:
        print("No objects found in choose file. Exiting.")
        return

    print(f"Found {len(objects)} objects in choose file")
